...
// ---------------------------------------------------------------------------
test('Modul×MA-Preismodell: Quote zeigt Mehrpreis + neuen Paketpreis, Upsell schaltet frei', async () => {
  const s = await register('Preis GmbH', 'preis@test.de');
  const A = { headers: { 'X-Admin-Token': 'test-admin-token' } };

  // START: genau 1 verkaufbares Modul (zeiten)
  await checkout(s.accessToken, 'START');
  let acc = await api('GET', '/api/account', { token: s.accessToken });
  assert.deepEqual(acc.data.tenant.modules, ['zeiten']);
  ...
  const bad = await api('POST', '/api/billing/module-quote', { token: s.accessToken, body: { module: 'quatsch' } });
  assert.equal(bad.status, 400);
});
